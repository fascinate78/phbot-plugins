...
def plugin_log(message):
    log("[FCaravanNavigator V3] {0}".format(message))


def client_notice(message):
    try:
        phBotChat.ClientNotice(
            "[FCaravanNavigator V3 | Made by FascinaTe] {0}".format(message)
        )
...
def load_settings():
    ...
    try:
        ...
        settings["discord_notifications_enabled"] = bool(
            settings["discord_notifications_enabled"]
        )
        settings["discord_webhook_url"] = str(
            settings["discord_webhook_url"]
        ).strip()
        settings["telegram_notifications_enabled"] = bool(
...
def discord_webhook_is_valid(webhook_url):
    webhook_url = str(webhook_url or "").strip()
    return (
        webhook_url.startswith("https://")
        and "/api/webhooks/" in webhook_url
    )


def build_discord_message(event_name, details):
    character_name = "Unknown"
    server_name = "Unknown"
    try:
        character = get_character_data()
        if isinstance(character, dict):
            character_name = character.get("name") or character_name
            server_name = character.get("server") or server_name
    except Exception:
        pass

    return (
        "**FCaravanNavigator V3 - {0}**\n"
        "Character: `{1}`\n"
        "Server: `{2}`\n"
        "{3}"
    ).format(event_name, character_name, server_name, details)


def _post_discord_webhook(webhook_url, message):
    try:
        ...
        request = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "FCaravanNavigator-V3"
            }
        )
        ...
        if status_code not in (200, 204):
            plugin_log("Discord notification failed with HTTP {0}".format(status_code))
    except urllib.error.HTTPError as error:
        plugin_log("Discord notification failed with HTTP {0}".format(error.code))
    except Exception as error:
        # Log only the exception type because some errors may contain the secret URL.
        plugin_log(
            "Could not send Discord notification ({0})".format(
                type(error).__name__
            )
        )


def send_discord_notification(event_name, details, force=False):
    if not force and not settings.get("discord_notifications_enabled", False):
        return False

    webhook_url = settings.get("discord_webhook_url", "").strip()
    if not discord_webhook_is_valid(webhook_url):
        plugin_log("Discord notification skipped: webhook URL is missing or invalid")
        return False

    message = build_discord_message(event_name, details)
    worker = threading.Thread(
        target=_post_discord_webhook,
...
def telegram_credentials_are_valid(bot_token, chat_id):
    bot_token = str(bot_token or "").strip()
    chat_id = str(chat_id or "").strip()
    return bool(bot_token and ":" in bot_token and chat_id)


def _post_telegram_message(bot_token, chat_id, message):
    endpoint = "https://api.telegram.org/bot{0}/sendMessage".format(bot_token)
    try:
        payload = json.dumps({
            "chat_id": chat_id,
            "text": message,
        ...
        try:
            ...
            response.close()
        if status_code != 200:
            plugin_log("Telegram notification failed with HTTP {0}".format(status_code))
    except urllib.error.HTTPError as error:
        try:
            error_body = error.read().decode("utf-8", errors="replace")
        except Exception as body_error:
            error_body = "<could not read response body: {0!r}>".format(body_error)
        plugin_log(
            "Telegram notification failed: endpoint={0}, HTTP {1}, reason={2!r}, "
            "response={3}".format(
                endpoint,
                error.code,
                error.reason,
        ...
        if "CERTIFICATE_VERIFY_FAILED" in reason_text:
            try:
                ...
                if status_code == 200:
                    plugin_log(
                        "Telegram notification sent using SSL compatibility mode"
                    )
                else:
                    plugin_log(
                        "Telegram SSL compatibility retry failed with HTTP {0}".format(
                            status_code
                        )
                    )
            except urllib.error.HTTPError as retry_error:
                try:
                    retry_body = retry_error.read().decode(
                        "utf-8",
                        errors="replace"
                    )
                except Exception as body_error:
                    retry_body = "<could not read response body: {0!r}>".format(
                        body_error
                    )
                plugin_log(
                    "Telegram SSL compatibility retry failed: endpoint={0}, "
                    "HTTP {1}, reason={2!r}, response={3}".format(
                        endpoint,
                        retry_error.code,
                        retry_error.reason,
                        retry_body
            ...
            plugin_log(
                "Telegram notification URL error: endpoint={0}, reason={1!r}, "
                "error={2!r}".format(
                    endpoint,
                    error.reason,
                    error
        ...
        plugin_log(
            "Could not send Telegram notification: endpoint={0}, type={1}, "
            "error={2!r}".format(
                endpoint,
                type(error).__name__,
                error
...
def send_telegram_notification(event_name, details, force=False):
    if not force and not settings.get("telegram_notifications_enabled", False):
        return False

    bot_token = settings.get("telegram_bot_token", "").strip()
    chat_id = settings.get("telegram_chat_id", "").strip()
    if not telegram_credentials_are_valid(bot_token, chat_id):
        plugin_log("Telegram notification skipped: token or Chat ID is invalid")
        return False

    message = build_discord_message(event_name, details).replace("**", "").replace("`", "")
    worker = threading.Thread(
        target=_post_telegram_message,
...
def send_event_notifications(event_name, details):
    send_discord_notification(event_name, details)
    send_telegram_notification(event_name, details)
...
def create_settings_gui():
    ...
    global telegram_settings_widgets
    ...
    global discord_enabled_checkbox
    global discord_webhook_edit
    global telegram_enabled_checkbox
    global telegram_token_edit
    global telegram_chat_id_edit
    ...
    def add_telegram_widget(widget, x, y):
        telegram_settings_widgets.append((widget, x, y))
        return widget
    ...
    discord_enabled_checkbox = add_widget(
        QtBind.createCheckBox(
            gui, "settings_changed", "Enable Discord", 12, 230
        ),
        12,
        230
    )
    add_widget(QtBind.createLabel(gui, "Webhook:", 145, 233), 145, 233)
    discord_webhook_edit = add_widget(
        QtBind.createLineEdit(gui, "", 205, 229, 180, 20), 205, 229
    )
    add_widget(
        QtBind.createButton(gui, "test_discord", "Test Discord", 340, 202),
        340,
        202
    )
    add_widget(
        QtBind.createButton(gui, "show_telegram_settings", "Telegram →", 300, 120),
        300,
        120
    )
    stop_character_checkbox = add_widget(
        QtBind.createCheckBox(
            gui, "settings_changed", "Stop on character death", 12, 260
        ),
        12,
        260
    )
    stop_transport_checkbox = add_widget(
        QtBind.createCheckBox(
            gui, "settings_changed", "Stop on transport death", 200, 260
        ),
        200,
        260
    )
    attacker_tracking_checkbox = add_widget(
        QtBind.createCheckBox(
            gui, "settings_changed", "Track last player attacker", 12, 286
        ),
        12,
        286
    )
    add_widget(QtBind.createLabel(gui, "Memory (sec):", 200, 289), 200, 289)
    attacker_seconds_edit = add_widget(
        QtBind.createLineEdit(gui, "15", 290, 285, 50, 20), 290, 285
    )
    ...
    add_telegram_widget(
        QtBind.createLabel(
            gui, '<font color="#5b57e0"><b>TELEGRAM NOTIFICATIONS</b></font>', 12, 122
        ),
        12,
        122
    )
    add_telegram_widget(
        QtBind.createButton(gui, "show_general_settings", "← General", 300, 120),
        300,
        120
    )
    add_telegram_widget(QtBind.createLineEdit(gui, "", 12, 142, 374, 1), 12, 142)
    telegram_enabled_checkbox = add_telegram_widget(
        QtBind.createCheckBox(
            gui, "settings_changed", "Enable Telegram notifications", 12, 154
        ),
        12,
        154
    )
    add_telegram_widget(QtBind.createLabel(gui, "Bot token:", 12, 184), 12, 184)
    telegram_token_edit = add_telegram_widget(
        QtBind.createLineEdit(gui, "", 90, 180, 296, 20), 90, 180
    )
    add_telegram_widget(QtBind.createLabel(gui, "Chat ID:", 12, 214), 12, 214)
    telegram_chat_id_edit = add_telegram_widget(
        QtBind.createLineEdit(gui, "", 90, 210, 180, 20), 90, 210
    )
    add_telegram_widget(
        QtBind.createButton(gui, "test_telegram", "Test Telegram", 12, 244),
        12,
        244
    )
    add_telegram_widget(
        QtBind.createButton(gui, "save_telegram_settings", "Save Telegram", 135, 244),
        135,
        244
    )
    for widget, x, y in telegram_settings_widgets:
        QtBind.move(gui, widget, OFFSCREEN_X, y)
...
def apply_settings_to_gui():
    ...
    QtBind.setChecked(
        gui,
        discord_enabled_checkbox,
        settings["discord_notifications_enabled"]
    )
    QtBind.setText(gui, discord_webhook_edit, settings["discord_webhook_url"])
    QtBind.setChecked(
        gui,
        telegram_enabled_checkbox,
        settings["telegram_notifications_enabled"]
    )
    QtBind.setText(gui, telegram_token_edit, settings["telegram_bot_token"])
    QtBind.setText(gui, telegram_chat_id_edit, settings["telegram_chat_id"])
...
def show_telegram_settings():
    ...
    QtBind.move(gui, destinations_button, 310, 62)
    apply_settings_to_gui()
    set_status("Telegram settings", C_ACCENT)
...
def save_settings():
    ...
    settings["discord_notifications_enabled"] = QtBind.isChecked(
        gui, discord_enabled_checkbox
    )
    webhook_url = QtBind.text(gui, discord_webhook_edit).strip()
    if webhook_url and not discord_webhook_is_valid(webhook_url):
        set_status("Invalid Discord webhook URL", C_ERROR)
        return
...
def test_discord():
    webhook_url = QtBind.text(gui, discord_webhook_edit).strip()
    if not discord_webhook_is_valid(webhook_url):
        set_status("Enter a valid Discord webhook URL", C_ERROR)
        return
    settings["discord_webhook_url"] = webhook_url
    if send_discord_notification(
        ...
        set_status("Discord test queued", C_SUCCESS)


def read_telegram_settings_from_gui(require_credentials=False):
    bot_token = QtBind.text(gui, telegram_token_edit).strip()
    chat_id = QtBind.text(gui, telegram_chat_id_edit).strip()
    enabled = QtBind.isChecked(gui, telegram_enabled_checkbox)
    if (enabled or require_credentials) and not telegram_credentials_are_valid(
        bot_token, chat_id
    ):
        set_status("Enter a valid Telegram token and Chat ID", C_ERROR)
        return False
    settings["telegram_notifications_enabled"] = enabled
...
def save_telegram_settings():
    if not read_telegram_settings_from_gui():
        return
    if save_settings_file():
        set_status("Telegram settings saved", C_SUCCESS)
    else:
        set_status("Could not save Telegram settings", C_ERROR)


def test_telegram():
    if not read_telegram_settings_from_gui(require_credentials=True):
        return
    if send_telegram_notification(
        "Test Notification",
        "Telegram bot configuration is working.",
        force=True
    ):
        set_status("Telegram test queued", C_SUCCESS)
...
def stop_for_incident(incident_name):
    ...
    incident_details = "Travel time: `{0}`".format(format_duration(elapsed_time))
    if attacker:
        incident_details += "\nLast player attacker: `{0}`".format(attacker)
    else:
        incident_details += "\nLast attacker: `Unknown / NPC`"
    if current_location is not None:
        incident_details += "\nDestination: `{0}`".format(current_location["name"])
    send_event_notifications(incident_name, incident_details)
...
def confirm_pending_transport_death():
    ...
    if original_region and current_region and original_region != current_region:
        ...
        return
    if settings.get("stop_on_transport_death", True):
        stop_for_incident("Transport died")
    else:
        details = "The active transport pet died."
        if current_location is not None:
            details += "\nDestination: `{0}`".format(current_location["name"])
        send_event_notifications("Transport died", details)
...
def handle_event(t, data):
    ...
    if t == EVENT_DIED:
        if settings.get("stop_on_character_death", True):
            stop_for_incident("Character died")
        else:
            details = "The character died."
            if current_location is not None:
                details += "\nDestination: `{0}`".format(current_location["name"])
            send_event_notifications("Character died", details)
    elif t == EVENT_TRANSPORT_DIED and (
        settings.get("stop_on_transport_death", True)
        or settings.get("discord_notifications_enabled", False)
    ):
